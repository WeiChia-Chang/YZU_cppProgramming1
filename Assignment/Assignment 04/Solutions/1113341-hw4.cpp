	...
	int adderCoef[], int adderExpon[], int adderSize)
{
	...
	while (addendSize != 0 && addendCoef[addendSize - 1] == 0)
	{
		addendSize--;
	}
	...
	int productCoef[], int productExpon[], int& productSize)
{
	...
	int j = 0;
	int k = 0;
	if (multiplicandCoef != 0 && multiplierCoef != 0) {
		for (j = 0; j < multiplierSize; j++) {
			for (i = 0, k = 0; i < multiplicandSize; i++, k++) {
				bufferExpon[k] = multiplicandExpon[i] + multiplierExpon[j];
				bufferCoef[k] = multiplicandCoef[i] * multiplierCoef[j];
			}
	...
	int remainderCoef[], int remainderExpon[], int& remainderSize)
{
	...
	for (int j = quotientSize - 1; j >= 0; j--) {
		if (quotientCoef[j] == 0) {
			for (int k = j; k <= quotientSize - 1; k++) {
				quotientExpon[k] = quotientExpon[k + 1];
				quotientCoef[k] = quotientCoef[k + 1]; 
			}
			quotientSize--;
		}
	}
