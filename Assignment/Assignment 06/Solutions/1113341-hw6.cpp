...
void output(string str, int* hugeInt, int size);
...
int main()
{
	...
	while (cin >> t >> a >> b)
	{
		...
		else
		{
			...
			//output("dividend",dividend,dividendSize);
			if (dividendSize > maxSize - baseSize)
				cout << "is not an integer with less than 100 digits.\n";
			else
			{
			...
			//output("divisor",divisor,divisorSize);
			...
			//output("dividend-1",dividend,dividendSize);
			//output("divisor-1",divisor,divisorSize);
	...
	int dividendSize, int divisorSize, int& quotientSize, int& remainderSize)
{
	...
	else
	{
		...
		int* buffer = new int[bufferSize]();
		for (int i = divisorSize - 1, j = bufferSize - 1; i >= 0; i--, j--)
		{
			buffer[j] = divisor[i];
		}
