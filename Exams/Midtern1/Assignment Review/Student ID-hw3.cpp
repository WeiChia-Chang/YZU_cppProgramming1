               ...
               int dividendDegree, int divisorDegree, int &quotientDegree, int &remainderDegree )
{
   ...
   int bufferDegree = divisorDegree;
   int buffer[arraySize]{};
   const int monomialDegree{ 1 };
   int monomial[monomialDegree]{};
   ...
   for (int i = quotientDegree; i >= 0; i--) {
       ...
       /*寫法1： for (int j = divisorDegree; j >= 0; j--) {
           buffer[i + j] = quotient[i] * divisor[j];
       } for (int j = remainderDegree; j >= 0; j--) {
           remainder[j] -= buffer[j];
       }*/

       /*寫法2*/
       bufferDegree = divisorDegree + i; /*bufferDegree = 商目前項次+除式項次 如果不寫這行 bufferDegree=20 送到減法 後面就會有問題*/
       ...
       /*while (quotientDegree != 0 && quotient[quotientDegree] == 0) {
           quotientDegree--;
       } 測資不會有quotient=0的情況發生*/

       /*while (remainderDegree != 0 && remainder[remainderDegree] == 0) {
           remainderDegree--;
       } 可有可無*/

       if (remainderDegree == 0 || divisorDegree > remainderDegree) break;

       i = remainderDegree - divisorDegree + 1; /*可有可無*/
   }
